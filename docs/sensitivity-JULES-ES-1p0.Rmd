...
oaatLinePlotMultiUnc <- function(X_oaat, fitlist, n_oaat, ynames, linecol, shadecol = 'darkgrey', ...){
  
  nr <- length(fitlist)
  nc <- ncol (X_oaat)
  
  par(mfrow = c(nr,nc), mar = c(0.2,0.2, 0.2, 0.2), oma = c(0.1, 14,12,0.1))
  
  for(j in 1:length(fitlist )){
    
    col.transp <- adjustcolor(shadecol, alpha = 0.5)
    
    y_oaat <- predict.km(fitlist[[j]], newdata = X_oaat, type = 'UK')
    ...
    for(i in 1:ncol(X_oaat)){
      ...
      plot(X_oaat[ix, i], y_oaat$mean[ix], ylim =  range(c(y_oaat$mean + (2 * y_oaat$sd), y_oaat$mean - 
                                                             (2 * y_oaat$sd))), type = 'n', axes = FALSE)
      polygon(x = c(X_oaat[ix, i], rev(X_oaat[ix, i])), y = c(y_oaat$mean[ix] - 
                                                                (2 * y_oaat$sd[ix]), rev(y_oaat$mean[ix] + (2 * y_oaat$sd[ix]))), 
              col = col.transp, border = col.transp)
      lines(X_oaat[ix, i], y_oaat$mean[ix], col = linecol)
      
      if(j==1){
        mtext(colnames(X_oaat)[i], side = 3, line = 1, las = 3)
      }
      
      if(i==1){
        mtext(ynames[j], side = 2, las = 1, line = 1)
...
oaat_level1a_Y_file <- 'data/oaat_level1a_Y_2023-07-13.rdata'
...
oaat_level1a_YAnom_file <- 'data/oaat_level1a_YAnom_2023-13-07.rdata'
...
First, "modern value" outputs (recent means)


```{r, fig.width = 12, fig.height = 4}
oaatLinePlotMultiUnc(X_oaat = X_oaat_level1a,
                     fitlist = fit_list_const_level1a,
                     n_oaat = 21,
                     ynames = colnames(Y_sum_level1a),
                     linecol = 'black', 
                     shadecol = 'darkgrey'
                     )
...
pdf(file = 'figs/figA07.pdf', width = 12, height = 12)
oaatLinePlotMultiUnc(X_oaat = X_oaat_level1a,
                     fitlist = emlist_km_Y_level1a,
                     n_oaat = 21,
                     ynames = colnames(Y_sum_level1a),
                     linecol = 'black', 
                     shadecol = 'darkgrey'
...
Second, "anomaly" values (recent minus initial)

```{r, fig.width = 12, fig.height = 12}
pdf(file = 'figs/figA08.pdf', width = 12, height = 12)
oaatLinePlotMultiUnc(X_oaat = X_oaat_level1a,
                     fitlist = emlist_km_YAnom_level1a,
                     n_oaat = 21,
                     ynames = colnames(Y_sum_level1a),
                     linecol = 'black', 
                     shadecol = 'darkgrey'
...
### How does uncertainty affect uncertainty analysis across all outputs?
...
oaatLinePlotMultiUnc <- function(X_oaat, Y_oaat, n_oaat, nr, nc, cols, ...){
...
oaatLineplotBasic <- function (X_oaat, y_oaat_mean, y_oaat_sd, n_oaat, nr, nc, linecol = "black", 
    shadecol = "grey", ...) 
{
    col.transp <- adjustcolor(shadecol, alpha = 0.5)
    ...
    for (i in 1:ncol(X_oaat)) {
        ...
        polygon(x = c(X_oaat[ix, i], rev(X_oaat[ix, i])), y = c(y_oaat_mean[ix] - 
            (2 * y_oaat$sd[ix]), rev(y_oaat_mean[ix] + (2 * y_oaat_sd[ix]))), 
            col = col.transp, border = col.transp)
...
pdf(file = 'figs/fig11.pdf', width = 10, height = 10)
...
emlist_km_Y_level1a_wave01_file <- "data/emlist_km_Y_level1a_wave01_2023-08-01.rdata" 
...
oaat_level1a_wave01_Y_file <- 'data/oaat_level1a_wave01_Y_2023-08-01.rdata'
...
pdf(file = 'figs/fig09.pdf', width = 7, height = 8)
...
emlist_km_Y_level1a_wave01_file <- 'data/emlist_km_Y_level1a_wave01_file_2023-08-01.rdata' 
...
pdf(file = 'figs/fig10.pdf', width = 7, height = 8)
...
MF_Y_sum_level1a_file <- 'data/MF_Y_sum_level1a_2023-08-01.rdata'
...
pdf(file = 'figs/figA09.pdf', width = 7, height = 8)
...
MF_YAnom_sum_level1a_file <- 'data/MF_YAnom_sum_level1a_2023-08-01.rdata'
...
# Becomes figure A10 for revised submission
pdf(file = 'figs/figA10.pdf', width = 7, height = 8)
...
pdf(file = 'figs/fig12.pdf', width = 6, height = 8 )
