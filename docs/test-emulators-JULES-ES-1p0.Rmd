...
emlist_km_Y_select_level1a_file <- "data/emlist_km_Y_select_level1a_2023-07-13.rdata"
...
for(i in 1:length(loolist_km_Y_select_level1a)){
  ...
  abline(0,1)
  legend('topleft', legend = colnames(Y_select_wave00_level1a)[i], bty = 'n', text.font = 2, cex = 1.3  )
  legend('bottomright',legend = paste('pmae =',round(loostats_km_Y_select_level1a[[i]]$pmae,2),'%') , bty = 'n', text.font = 2, cex = 1.3)
...
emlist_km_YAnom_select_level1a_file <- "data/emlist_km_YAnom_select_level1a_2023-07-13.rdata"
...
emlist_km_Y_select_wave00_level1a_wave01_file <- "data/emlist_km_Y_select_wave00_level1a_wave01_2023-07-13.rdata"
...
for(i in 1:length(loolist_km_Y_select_wave00_level1a_wave01)){
  ...
  abline(0,1)
  legend('topleft', legend = colnames(Y_select_wave00_level1a)[i], bty = 'n', text.font = 2, cex = 1.3  )
  legend('bottomright',legend = paste('pmae =',round(loostats_km_Y_select_wave00_level1a_wave01[[i]]$pmae,2),'%') ,
         bty = 'n', text.font = 2, cex = 1.3)
...
Are the uncertainty estimates of the emulator consistent with the observed errors?
Use a rank histogram method.
...
```{r}
 ...
 # Rank histograms for checking the uncertainty.
  ...
  looRankHist = function(obs, pred_mean, pred_sd, n = 1000){
    # a version of the rank histogram     
    obs_ranks = rep(NA, length(obs))
    
    for(i in 1:length(obs)){
      ranks = rank(c(obs[i], rnorm(n = n, mean = pred_mean[i], sd = pred_sd[i])))
      obs_ranks[i] = ranks[1]
    }
...
pdf(file = 'figs/figA04.pdf', width = 10, height = 8)
par( mfrow = c(5,3), mar = c(5,5,3,2))

for(i in 1:length(emlist_km_Y_select_wave00_level1a_wave01)){
  
  y <- emlist_km_Y_select_wave00_level1a_wave01[[i]]@y
  loo <- loolist_km_Y_select_wave00_level1a_wave01[[i]]
  
  loo_rank_hist <-  looRankHist(obs = y,
                                pred_mean = loo$mean,
                                pred_sd = loo$sd,
                                n = 1000)
  ...
  par(las = 1, fg = 'white')
  hist(loo_rank_hist, col = 'grey', 
       axes = FALSE, main = colnames(Y_select_wave00_level1a)[i],
       xlab = 'Rank', cex.main = 1.5
  )
  axis(1, col = 'black')
  axis(2, col = 'black')
...
emlist_km_YAnom_select_wave00_level1a_wave01_file <- "data/emlist_km_YAnom_select_wave00_level1a_wave01_2023-07-13.rdata"
...
for(i in 1:length(loolist_km_YAnom_select_wave00_level1a_wave01)){
  ...
  segments(x0 = y, y0 = loo$mean - (2*loo$sd)  , x1 = y , y1 = loo$mean + (2*loo$sd), col = makeTransparent(wave00_wave01_colvec, 30))
  abline(0,1)
  legend('topleft', legend = colnames(YAnom_select_wave00_level1a)[i], bty = 'n', text.font = 2, cex = 1.3  )
  legend('bottomright',legend = paste('pmae =',round(loostats_km_YAnom_select_wave00_level1a_wave01[[i]]$pmae,2),'%') , bty = 'n', text.font = 2, cex = 1.3)
...
pdf(file = 'figs/figA05.pdf', width = 10, height = 8)
par( mfrow = c(5,3), mar = c(5,5,4,2))

for(i in 1:length(emlist_km_YAnom_select_wave00_level1a_wave01)){
  ...
  par(las = 1, fg = 'white')
  hist(loo_rank_hist, col = 'grey', 
       axes = FALSE, main = colnames(YAnom_select_wave00_level1a)[i],
       xlab = 'Rank', cex.main = 1.5
  )
  axis(1, col = 'black')
  axis(2, col = 'black')
...
# Becomes A05 for revised submission
pdf(file = 'figs/figA03.pdf', width = 5, height = 10)
...
for(i in 1:length(loolist_km_Y_level1a)){
  ...
  segments(x0 = y, y0 = loo$mean - (2*loo$sd)  , x1 = y , y1 = loo$mean + (2*loo$sd), col = makeTransparent(wave00col, 50))
  abline(0,1)
  legend('topleft', legend = colnames(Y_const_level1a_scaled)[i], bty = 'n', text.font = 2, cex = 1.3  )
  legend('bottomright',legend = paste('pmae =',round(loostats_km_Y_level1a[[i]]$pmae,2),'%') , bty = 'n', text.font = 2, cex = 1.3)
...
for(i in 1:length(loolist_km_Y_level1a)){
  ...
  abline(0,1)
  legend('topleft', legend = colnames(Y_const_level1a_scaled)[i], bty = 'n', text.font = 2, cex = 1.3  )
  legend('bottomright',legend = paste('pmae =',round(loostats_km_Y_level1a_wave01[[i]]$pmae,2),'%') , bty = 'n', text.font = 2, cex = 1.3)
...
for(i in 1:length(loolist_km_Y_level1a_level2)){
  ...
  points(preddiff_wave01[loo_rank_ix$ix], xlab = '', ylab = '', main = '' , col = makeTransparent(wave01col, 255),
   ...
   mtext(colnames(Y_const_level1a_scaled)[i], side = 3, adj = 0, line = 1,  cex = 1.3)
...
unc_km_Y_const_level1a_scaled <- matrix(ncol = ncol(Y_const_level1a_scaled), nrow = nrow(Y_const_level1a_scaled))
colnames(unc_km_Y_const_level1a_scaled) <- colnames(Y_const_level1a_scaled)

for(i in 1:ncol(Y_const_level1a_scaled)){
  
  unc <- loolist_km_Y_const_level1a_scaled[[i]]$sd
  unc_km_Y_const_level1a_scaled[, i] <- unc
...
# Becomes figure A06 for revised submission
pdf(width = 10, height = 10, file = 'figs/figA06.pdf')
...
plot(log(Y_const_level1a_scaled[ ,4]), log(pred_km_Y_const_level1a_scaled[ ,4]), type = 'n', las = 1, main = colnames(Y_const_level1a_scaled)[4],
     xlab = 'model', ylab = 'emulator')
points(log(Y_const_level1a_scaled[ ,4]), log(pred_km_Y_const_level1a_scaled[ ,4]), col = colvec, pch = pchvec)
```
...
plot(y, loo_m_stan$mean,col = makeTransparent('black', 150), pch = 19, xlim = c(-200, 2100), ylim = c(-200, 2100))
...
legend('topleft', col = c(makeTransparent('black', 150), makeTransparent('red', 150),makeTransparent('blue', 150)),
       legend = c('standard', 'log(y)', 'sqrt(y)'),
       pch = 19,
       title = 'y transformation')
...
Taking the square root of the output improves the emulator - especially at very low veg Carbon (i.e unrealistically close to zero)
```{r}
...
However, the emulator doesn't markedly improve the predictions as to which ensemble members would fall within the NROY limits. Same percent correct.
```{r}
ver$tab
...
pred_km_Y_const_level1a_scaled_cVeg_sqrt_gauss_gen <- pred_km_Y_const_level1a_scaled
pred_km_Y_const_level1a_scaled_cVeg_sqrt_gauss_gen[, 'cVeg_lnd_sum'] <- (loo_m_sqrt_gauss_gen$mean)^2

pred_level2_sqrt_gauss_gen_ix <- withinConstraints(pred_km_Y_const_level1a_scaled_cVeg_sqrt_gauss_gen, AW_const)


emulator_sqrt_gauss_gen <- vector(mode = "logical", length = nrow(Y_const_level1a_scaled))
emulator_sqrt_gauss_gen[pred_level2_sqrt_gauss_gen_ix] <- TRUE
ver_sqrt_gauss_gen <- verify(obs = model, pred = emulator_sqrt_gauss_gen, frcst.type = 'binary')
